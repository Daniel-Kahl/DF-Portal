  ...
  return (
    <div>
      <Accordion collapseAll>
        <Accordion.Panel>
          <Accordion.Title
            className="bg-[#000000] text-[#c4b6b6]"
            style={{
              display: "flex",
              flexDirection: "row",
            }}
          >
            ...
            <div
              className=""
              style={{ color: rarityColors.get(`${equipItem.itemRarity}`) }}
            >
...
function AllItemimage({ itemimage }: { itemimage: Models.Equipment }) {
  return (
    ...
    >
      <div
        className="equipmentImageContainer m-1 p-1"
        style={{
          display: "flex",
        }}
      >
...
function Option5({ option5 }: { option5: Models.Option5 }) {
  return <div>{`${option5.explainDetail}`}</div>;
}
// enchants
  ...
  if (upgrade.refine != 0) {
    return (
      ...
      >
        ...
        <span style={{ color: "#800080" }}>{`${upgrade.refine} `}</span>
